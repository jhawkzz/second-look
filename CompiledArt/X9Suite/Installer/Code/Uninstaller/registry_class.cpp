...
BOOL Registry_Class::Open_Key( const char * key, BOOL createOnFail /*=TRUE*/ )
{
   BOOL success = TRUE;
   ...
   if ( RegOpenKeyEx( m_root_key, key, 0, KEY_ALL_ACCESS, &m_key_handle ) != ERROR_SUCCESS )
   {
      success = FALSE;
   }
...
BOOL Registry_Class::Get_Value_Data_Binary( char * value, BYTE * data, DWORD buffer_length, const BYTE * default_data )
{
   BOOL success = FALSE;
   ...
   do
   {
      ...
      if ( RegQueryValueEx( m_key_handle, value, 0, &type, data, &buffer_length ) != ERROR_SUCCESS ) break;
...
BOOL Registry_Class::Set_Value_Data_Binary( char * value, BYTE * data, uint32 size )
{
   BOOL success = FALSE;
   ...
   do
   {
      ...
      if ( RegSetValueEx( m_key_handle, value, 0, REG_BINARY, data, size ) != ERROR_SUCCESS ) break;
...
{
   if ( RegCreateKeyEx( m_root_key, key, 0, NULL, REG_OPTION_NON_VOLATILE, NULL, NULL, &m_key_handle, NULL ) != ERROR_SUCCESS ) 
   {
      return FALSE;
   }
