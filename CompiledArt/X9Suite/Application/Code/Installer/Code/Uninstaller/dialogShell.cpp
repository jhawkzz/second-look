...
DialogShell::DialogShell ( void )
{
   ...
   m_CurrentPage    = PageType_Confirm;
}
...
LRESULT CALLBACK DialogShell::Callback( HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
   ...
   if ( pDialogShell || WM_INITDIALOG == uMsg )
   {
      switch( uMsg )
      {
         ...
         case WM_UNINSTALL_FAILED: return pDialogShell->DialogShell_WM_UNINSTALL_FAILED( hwnd );
      }
   }
...
BOOL DialogShell::DialogShell_WM_UNINSTALL_FAILED( HWND hwnd )
{
   m_NoPromptToExit = TRUE;
...
{
   //create our windows
   m_pPageList[ 0 ] = new ConfirmPage;
   m_pPageList[ 1 ] = new UninstallPage;
   m_pPageList[ 2 ] = new FinishPage;
}
